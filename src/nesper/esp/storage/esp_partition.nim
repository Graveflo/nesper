...
type
  esp_partition_type_t* {.size: sizeof(cint).} = enum
    ESP_PARTITION_TYPE_APP = 0x00000000, ## !< Application partition type
...
type
  esp_partition_subtype_t* {.size: sizeof(cint).} = enum
    ESP_PARTITION_SUBTYPE_APP_FACTORY = 0x00000000, ## !< Factory application partition
    ESP_PARTITION_SUBTYPE_DATA_PHY = 0x00000001, ## !< PHY init data partition
    ESP_PARTITION_SUBTYPE_DATA_NVS = 0x00000002, ## !< NVS partition
    ...
    ESP_PARTITION_SUBTYPE_DATA_SPIFFS = 0x00000082, ## !< SPIFFS partition
...
# type
  # esp_partition_iterator_opaque_* {.importc: "$1", header: chdr, incompleteStruct.} = object
...
type
  esp_partition_t* {.importc: "esp_partition_t", header: "esp_partition.h", bycopy.} = object
...
proc esp_partition_register_external*(flash_chip: ptr esp_flash_t; offset: csize_t;
