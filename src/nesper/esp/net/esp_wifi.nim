...
const
  ...
  ESP_ERR_WIFI_NOT_STARTED* = (ESP_ERR_WIFI_BASE + 2) ## !< WiFi driver was not started by esp_wifi_start
  ESP_ERR_WIFI_NOT_STOPPED* = (ESP_ERR_WIFI_BASE + 3) ## !< WiFi driver was not stopped by esp_wifi_stop
  ESP_ERR_WIFI_IF* = (ESP_ERR_WIFI_BASE + 4) ## !< WiFi interface error
  ESP_ERR_WIFI_MODE* = (ESP_ERR_WIFI_BASE + 5) ## !< WiFi mode error
  ESP_ERR_WIFI_STATE* = (ESP_ERR_WIFI_BASE + 6) ## !< WiFi internal state error
  ESP_ERR_WIFI_CONN* = (ESP_ERR_WIFI_BASE + 7) ## !< WiFi internal control block of station or soft-AP error
  ESP_ERR_WIFI_NVS* = (ESP_ERR_WIFI_BASE + 8) ## !< WiFi internal NVS module error
  ESP_ERR_WIFI_MAC* = (ESP_ERR_WIFI_BASE + 9) ## !< MAC address is invalid
  ESP_ERR_WIFI_SSID* = (ESP_ERR_WIFI_BASE + 10) ## !< SSID is invalid
  ESP_ERR_WIFI_PASSWORD* = (ESP_ERR_WIFI_BASE + 11) ## !< Password is invalid
  ESP_ERR_WIFI_TIMEOUT* = (ESP_ERR_WIFI_BASE + 12) ## !< Timeout error
  ESP_ERR_WIFI_WAKE_FAIL* = (ESP_ERR_WIFI_BASE + 13) ## !< WiFi is in sleep state(RF closed) and wakeup fail
  ESP_ERR_WIFI_WOULD_BLOCK* = (ESP_ERR_WIFI_BASE + 14) ## !< The caller would block
  ESP_ERR_WIFI_NOT_CONNECT* = (ESP_ERR_WIFI_BASE + 15) ## !< Station still in disconnect status
  ESP_ERR_WIFI_POST* = (ESP_ERR_WIFI_BASE + 18) ## !< Failed to post the event to WiFi task
  ESP_ERR_WIFI_INIT_STATE* = (ESP_ERR_WIFI_BASE + 19) ## !< Invalod WiFi state when init/deinit is called
  ESP_ERR_WIFI_STOP_STATE* = (ESP_ERR_WIFI_BASE + 20) ## !< Returned when WiFi is stopping
