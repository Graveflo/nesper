...
proc esp_err_to_name*(code: EspErrorCode): cstring {.cdecl, importc: "esp_err_to_name",
    header: "freertos/FreeRTOS.h".}
proc esp_err_to_name_r*(code: EspErrorCode; buf: cstring; buflen: csize_t): cstring {.cdecl,
    importc: "esp_err_to_name_r", header: "freertos/FreeRTOS.h".}

proc doCheck*(ret: EspErrorCode) =
  if ret != ESP_OK:
...
proc ESP_ERROR_CHECK*(x: EspErrorCode) {.cdecl, importc: "ESP_ERROR_CHECK", header: "freertos/FreeRTOS.h".}
proc ESP_ERROR_CHECK_WITHOUT_ABORT*(x: EspErrorCode) {.cdecl,
  importc: "ESP_ERROR_CHECK_WITHOUT_ABORT", header: "freertos/FreeRTOS.h".}

#define ESP_LOGI( tag, format, ... )  
#define LOG_FORMAT(letter, format)  LOG_COLOR_ ## letter #letter " (%d) %s: " format LOG_RESET_COLOR "\n"

proc newEspError*[E](msg: string, error: EspErrorCode): ref E =
  new(result)
  result.msg = msg
  result.code = error
